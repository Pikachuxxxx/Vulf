...
{
    vkDestroyImageView(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), view, nullptr);
    vkDestroyImage(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), image, nullptr);
    vkFreeMemory(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), deviceMemory, nullptr);
    vkDestroySampler(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), sampler, nullptr);
}
...
void vkglTF::Texture::fromglTfImage(tinygltf::Image &gltfimage, std::string path, VkQueue copyQueue)
{
	...
	if (!isKtx) {
		...
		vkGetPhysicalDeviceFormatProperties(VKLogicalDevice::GetDeviceManager()->GetGPUManager().GetGPU(), format, &formatProperties);
		...
		VK_CALL(vkCreateBuffer(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), &bufferCreateInfo, nullptr, &stagingBuffer));
		vkGetBufferMemoryRequirements(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), stagingBuffer, &memReqs);
		memAllocInfo.allocationSize = memReqs.size;
		memAllocInfo.memoryTypeIndex = VKLogicalDevice::GetDeviceManager()->GetGPUManager().FindMemoryTypeIndex(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
		VK_CALL(vkAllocateMemory(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), &memAllocInfo, nullptr, &stagingMemory));
		VK_CALL(vkBindBufferMemory(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), stagingBuffer, stagingMemory, 0));

		uint8_t* data;
		VK_CALL(vkMapMemory(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), stagingMemory, 0, memReqs.size, 0, (void**)&data));
		memcpy(data, buffer, bufferSize);
		vkUnmapMemory(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), stagingMemory);
		...
		VK_CALL(vkCreateImage(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), &imageCreateInfo, nullptr, &image));
		vkGetImageMemoryRequirements(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), image, &memReqs);
		memAllocInfo.allocationSize = memReqs.size;
		memAllocInfo.memoryTypeIndex = VKLogicalDevice::GetDeviceManager()->GetGPUManager().FindMemoryTypeIndex(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
		VK_CALL(vkAllocateMemory(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), &memAllocInfo, nullptr, &deviceMemory));
		VK_CALL(vkBindImageMemory(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), image, deviceMemory, 0));

		VkCommandBuffer copyCmd = VKLogicalDevice::GetDeviceManager()->createCmdBuffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY, true);
		...
		VKLogicalDevice::GetDeviceManager()->flushCommandBuffer(copyCmd, copyQueue, true);

		vkFreeMemory(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), stagingMemory, nullptr);
		vkDestroyBuffer(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), stagingBuffer, nullptr);

		// Generate the mip chain (glTF uses jpg and png, so we need to create this manually)
		VkCommandBuffer blitCmd = VKLogicalDevice::GetDeviceManager()->createCmdBuffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY, true);
		...
		VKLogicalDevice::GetDeviceManager()->flushCommandBuffer(blitCmd, copyQueue, true);
	}
	else {
		...
		*/
		...
		VKLogicalDevice::GetDeviceManager()->flushCommandBuffer(copyCmd, copyQueue);
	...
	VK_CALL(vkCreateSampler(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), &samplerInfo, nullptr, &sampler));
	...
	VK_CALL(vkCreateImageView(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), &viewInfo, nullptr, &view));
...
void vkglTF::Material::createDescriptorSet(VkDescriptorPool descriptorPool, VkDescriptorSetLayout descriptorSetLayout, uint32_t descriptorBindingFlags)
{
	...
	VK_CALL(vkAllocateDescriptorSets(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), &descriptorSetAllocInfo, &descriptorSet));
	...
	vkUpdateDescriptorSets(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, nullptr);
}
...
vkglTF::Mesh::Mesh( glm::mat4 matrix) {
	...
	VK_CALL(VKLogicalDevice::GetDeviceManager()->createBuffer(
		...
		&uniformBlock));
	VK_CALL(vkMapMemory(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), uniformBuffer.memory, 0, sizeof(uniformBlock), 0, &uniformBuffer.mapped));
	uniformBuffer.descriptor = { uniformBuffer.buffer, 0, sizeof(uniformBlock) };
};

vkglTF::Mesh::~Mesh() {
	vkDestroyBuffer(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), uniformBuffer.buffer, nullptr);
	vkFreeMemory(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), uniformBuffer.memory, nullptr);
}
...
void vkglTF::Model::createEmptyTexture(VkQueue transferQueue)
{
	...
	imageCreateInfo.usage = VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
	VK_CALL(vkCreateImage(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), &imageCreateInfo, nullptr, &emptyTexture.image));

	vkGetImageMemoryRequirements(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), emptyTexture.image, &memReqs);
	memAllocInfo.allocationSize = memReqs.size;
	memAllocInfo.memoryTypeIndex = VKLogicalDevice::GetDeviceManager()->GetGPUManager().FindMemoryTypeIndex(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
	VK_CALL(vkAllocateMemory(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), &memAllocInfo, nullptr, &emptyTexture.deviceMemory));
	VK_CALL(vkBindImageMemory(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), emptyTexture.image, emptyTexture.deviceMemory, 0));
	...
	VKLogicalDevice::GetDeviceManager()->flushCommandBuffer(copyCmd, transferQueue);
	...
	VK_CALL(vkCreateSampler(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), &samplerCreateInfo, nullptr, &emptyTexture.sampler));
	...
	VK_CALL(vkCreateImageView(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), &viewCreateInfo, nullptr, &emptyTexture.view));
...
vkglTF::Model::~Model()
{
	vkDestroyBuffer(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), vertices.buffer, nullptr);
	vkFreeMemory(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), vertices.memory, nullptr);
	vkDestroyBuffer(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), indices.buffer, nullptr);
	vkFreeMemory(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), indices.memory, nullptr);
	...
	if (descriptorSetLayoutUbo != VK_NULL_HANDLE) {
		vkDestroyDescriptorSetLayout(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), descriptorSetLayoutUbo, nullptr);
		descriptorSetLayoutUbo = VK_NULL_HANDLE;
	}
	if (descriptorSetLayoutImage != VK_NULL_HANDLE) {
		vkDestroyDescriptorSetLayout(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), descriptorSetLayoutImage, nullptr);
		descriptorSetLayoutImage = VK_NULL_HANDLE;
	}
	vkDestroyDescriptorPool(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), descriptorPool, nullptr);
...
void vkglTF::Model::loadFromFile(std::string filename, VkQueue transferQueue, uint32_t fileLoadingFlags, float scale)
{
	...
	VKLogicalDevice::GetDeviceManager()->flushCommandBuffer(copyCmd, transferQueue, true);

	vkDestroyBuffer(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), vertexStaging.buffer, nullptr);
	vkFreeMemory(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), vertexStaging.memory, nullptr);
	vkDestroyBuffer(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), indexStaging.buffer, nullptr);
	vkFreeMemory(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), indexStaging.memory, nullptr);
	...
	VK_CALL(vkCreateDescriptorPool(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), &descriptorPoolCI, nullptr, &descriptorPool));
	...
	// Descriptors for per-node uniform buffers
	{
		...
		if (descriptorSetLayoutUbo == VK_NULL_HANDLE) {
			...
			VK_CALL(vkCreateDescriptorSetLayout(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), &descriptorLayoutCI, nullptr, &descriptorSetLayoutUbo));
		}
		for (auto node : nodes) {
	...
	// Descriptors for per-material images
	{
		...
		if (descriptorSetLayoutImage == VK_NULL_HANDLE) {
			...
			descriptorLayoutCI.bindingCount = static_cast<uint32_t>(setLayoutBindings.size());
			descriptorLayoutCI.pBindings = setLayoutBindings.data();
			VK_CALL(vkCreateDescriptorSetLayout(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), &descriptorLayoutCI, nullptr, &descriptorSetLayoutImage));
		}
		for (auto& material : materials) {
			if (material.baseColorTexture != nullptr) {
...
void vkglTF::Model::prepareNodeDescriptor(vkglTF::Node* node, VkDescriptorSetLayout descriptorSetLayout) {
	if (node->mesh) {
		...
		descriptorSetAllocInfo.pSetLayouts = &descriptorSetLayout;
		descriptorSetAllocInfo.descriptorSetCount = 1;
		VK_CALL(vkAllocateDescriptorSets(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), &descriptorSetAllocInfo, &node->mesh->uniformBuffer.descriptorSet));
		...
		vkUpdateDescriptorSets(VKLogicalDevice::GetDeviceManager()->GetLogicalDevice(), 1, &writeDescriptorSet, 0, nullptr);
	}
	for (auto& child : node->children) {
